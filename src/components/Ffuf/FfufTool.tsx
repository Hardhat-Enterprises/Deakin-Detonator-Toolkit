import { Button, Stack, TextInput, Group, Switch, Alert } from "@mantine/core";
...
const FfufTool = () => {
    ...
    const handleProcessData = useCallback((data: string) => {
        if (/caught keyboard interrupt|ctrl-c|\^c/i.test(data)) {
    ...
    const handleProcessTermination = useCallback(
        ({ code, signal }: { code: number; signal: number }) => {
            if (interruptedRef.current || signal === 15 || signal === 2) {
