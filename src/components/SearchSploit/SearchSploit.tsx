    ...
    return (
        <form
            onSubmit={form.onSubmit((values) =>
                onSubmit({ ...values, searchOption: selectedSearchOption, out_put: selectedout_putOption, non_search: selectedNonSearchOption})
            )}
        >
            ...
            <Stack>
                ...
                <NativeSelect
                    ...
                    description={"Mirror (aka copies) an exploit to the current working directory, Examine (aka opens) the exploit using $PAGER"}
                />
     
                <TextInput label={"EBD-ID"} description="Exploit Database ID: Required when using the 'Path' output, or when using Non-Search options." {...form.getInputProps("ebd_id")} />
                ...
                <Accordion.Item value="item-1">
                    ...
                    <List>
                        ...
                        <List.Item>Exact | Perform an EXACT & order match on exploit title (Default is an AND match on each term).</List.Item>
                        <List.Item>Strict |  Perform a strict search, so input values must exist, disabling fuzzy search for version range.</List.Item>
                        <List.Item>Title | Search JUST the exploit title (Default is title AND the file's path).</List.Item>
                    </List>
                    <br></br>
                    <List>
                        ...
                        <List.Item>Path | Show the full path to an exploit (and also copies the path to the clipboard if possible).</List.Item>
                    ...
                    <List>
                        ...
                        <List.Item>Mirror | Mirror (aka copies) an exploit to the current working directory.</List.Item>
