...
const NetcatTool = () => {
  ...
  const tutorial =
    "https://docs.google.com/document/d/1NQ-hy8NBuTTUJzHebST5UF42JPjJ3yfIvNgWbM7FPLE/edit?usp=sharing";
  ...
  const onSubmit = async (values: FormValuesType) => {
    ...
    switch (values.netcatOptions) {
      ...
      case "Receive File":
        command = "bash";
        args = ["-c", `nc -l${verboseFlagWithSpaceAndDash} -n -w 60 -p ${values.portNumber} > "${values.filePath}"`];
        break;
      default:
          ...
          <Stepper.Step label="Provide Inputs">
            <Stack>
              ...
              {form.values.netcatOptions === "Listen" && (
                <TextInput label={"Port number"} required {...form.getInputProps("portNumber")} error={form.errors.portNumber} />
              )}
              ...
              {form.values.netcatOptions === "Receive File" && (
                <>
                  <TextInput label={"Port number"} required {...form.getInputProps("portNumber")} error={form.errors.portNumber} />
                  <TextInput label={"File path"} required {...form.getInputProps("filePath")} error={form.errors.filePath} />
                </>
              )}
