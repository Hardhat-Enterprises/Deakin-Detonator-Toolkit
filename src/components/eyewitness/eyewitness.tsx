...
const tutorial =
  "https://docs.google.com/document/d/1V4lIQbeIbKwNiLQqSXvJZ0HM34q5FDxe2HHP2t6d8mA/edit?usp=sharing"; // Link to the official tutorial/documentation.
...
function parseSummary(raw: string) {
  ...
  const get = (rx: RegExp) =>
    (lines.find((l) => rx.test(l)) || "").split(":").slice(1).join(":").trim();
  return {
...
function Eyewitness() {
  ...
  const validatePaths = useCallback(async () => {
    ...
    try {
      ...
      if (input.startsWith(outdir)) {
        setFriendlyError(
          "Input file and output folder cannot be the same location.",
        );
        return;
      }
  ...
  const handleProcessTermination = useCallback(
    async ({ code, signal }: { code: number; signal: number }) => {
      if (code === 0) {
        ...
        else if (/empty file|zero length/.test(raw))
          setFriendlyError("Input file is empty");
        else
          setFriendlyError(
            "Execution failed. Please check inputs and try again.",
          );
      }
  ...
  const onSubmit = async (values: FormValuesType) => {
    ...
    if (!values.directory.startsWith("/")) {
      setLoading(false);
      setOutput(
        "Error: Output directory must be an absolute path (start with /).",
      );
      return;
    }
  ...
  const summary = useMemo(
    () => (output ? parseSummary(output) : null),
    [output],
  );
        ...
        <Stack>
          ...
          {showAlert && (
            <Alert title="Warning: Potential Risks" color="red">
              This tool is used to perform website enumeration, use with caution
              and only on targets you own or have explicit permission to test.
            </Alert>
          )}
