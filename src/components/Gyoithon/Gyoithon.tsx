...
const Gyoithon = () => {
    ...
    // GyoiThon is not an APT package. It is installed under /usr/share/ddt/GyoiThon
    // and uses its own Python virtual environment.
    const gyoiThonPath = "/usr/share/ddt/GyoiThon";
    const gyoiPython = "gyoithon-python";
    const requirementsPath = `${gyoiThonPath}/requirements.txt`;
    ...
    const Install = async () => {
        ...
        try {
            // Install all requirements into GyoiThon's own virtual environment.
            // This avoids installing packages into the system/pyenv Python.
            const installOutput = await CommandHelper.runCommand(gyoiPython, [
                "-m",
                "pip",
                "install",
                "-r",
            ...
            setOutput(installOutput + "\nGyoiThon dependencies installed successfully.");
            setValue("configure");
        } catch (error) {
            setOutput(`Failed to install GyoiThon dependencies: ${String(error)}`);
        } finally {
            setLoading(false);
        }
    };
    ...
    const ClearTarget = async () => {
        ...
        const output = await CommandHelper.runCommand(gyoiPython, args);
        setOutput(output);
        setLoading(false);
    };
        ...
        >
            <p>
                ...
                <Stack>
                    ...
                    ></Alert>
                    <Accordion variant="contained" value={value} onChange={(newValue) => setValue(newValue)}>
